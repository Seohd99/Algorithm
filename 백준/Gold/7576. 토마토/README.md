...
### 문제점
느린 큐 연산
처음에는 BFS 큐를 구현할 때 `list`를 사용해서 `Q.pop(0)` 방식 사용
로직 자체는 잘 동작했지만, 제출시 시간초과 발생
알고 보니 `list.pop(0)`은 **시간 복잡도가 O(n)** 이기 때문에, 큐의 길이가 길어질수록 성능이 급격히 저하

### 해결 방법: deque로 최적화

파이썬의 `collections.deque`를 사용하면 `popleft()`로 앞에서 꺼내는 연산을 **O(1)** 시간에 처리할 수 있어, BFS에 적합
결과적으로 `deque`를 사용한 후에는 성능 문제 해결
